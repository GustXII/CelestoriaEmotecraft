...
public class ModernChooseWheel implements IChooseWheel {
    ...
    private static final int MAX_PAGE = 99; // 100 pages (0-99)
    ...
    public void render(GuiGraphics matrices, int mouseX, int mouseY, float delta){
        ...
        // แสดง page เป็น "1/100" แทน "1"
        Component text = Component.literal((fastMenuPage + 1) + "/" + (MAX_PAGE + 1));
        int textWidth = Minecraft.getInstance().font.width(text);
        matrices.drawString(Minecraft.getInstance().font, text, (int) (widget.x + widget.size / 2f - textWidth / 2f), (int) (widget.y + widget.size / 2f - 3), -1);
    }
            ...
            if(element != null){
                ...
                if (selectedPageButton == 0) {
                    ...
                    if (fastMenuPage > 0) {
                        ...
                        fastMenuPage = MAX_PAGE;
                    }
                } else if (selectedPageButton == 1) {
                    // ปุ่มขวา: ไปหน้าถัดไป
                    if (fastMenuPage < MAX_PAGE) {
                        fastMenuPage += 1;
                    } else {
                        fastMenuPage = 0;
                    }
    ...
    protected class FastChooseElement implements IChooseWheel.IChooseElement {
        ...
        public void renderHover(GuiGraphics matrices, ResourceLocation t){
            switch (id) {
                case 0:
                    drawTexture_select(matrices, t,0, 256, 0, 384, 2, 1);
                    break;
                case 1:
                    drawTexture_select(matrices, t,256, 256, 384, 384, 1, 1);
                    break;
                case 2:
                    drawTexture_select(matrices, t,256, 0, 384, 0, 1, 2);
                    break;
                case 3:
                    drawTexture_select(matrices, t,256, 0, 384, 256, 1, 1);
                    break;
                case 4:
                    drawTexture_select(matrices, t,0, 0, 0, 256, 2, 1);
                    break;
                case 5:
                    drawTexture_select(matrices, t,0, 0, 256, 256, 1, 1);
                    break;
                case 6:
                    drawTexture_select(matrices, t,0, 0, 256, 0, 1, 2);
                    break;
                case 7:
                    drawTexture_select(matrices, t,0, 256, 256, 384, 1, 1);
                    break;
            }
        }
    }
